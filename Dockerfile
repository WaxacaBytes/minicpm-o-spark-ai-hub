...
MiniCPM-o's remote modeling code does top-level `from flash_attn import flash_attn_func`.
We return a callable that raises only if actually invoked; sdpa is the real path.
"""
def _stub(*a, **kw):
    raise RuntimeError("flash_attn was called but is stubbed; use attn_implementation='sdpa'")
